// File: components/merchant-modules/MidlineQCPanel.tsx
// Phase 9: Midline/Endline QC - Full CRUD Component
...
import type { MidlineEndlineInspection } from '@/lib/merchant-types';
import { merchantStore } from '@/lib/merchant-store';

export function MidlineQCPanel() {
  const [inspections, setInspections] = useState<MidlineEndlineInspection[]>([]);
  const [workOrders, setWorkOrders] = useState<any[]>([]);
  const [showForm, setShowForm] = useState(false);
  const [editingInspection, setEditingInspection] = useState<MidlineEndlineInspection | null>(null);
  const [formData, setFormData] = useState<Partial<MidlineEndlineInspection>>({});
  ...
  const loadInspections = () => {
    setInspections(merchantStore.getAllMidlineEndlineInspections?.() || []);
  };
  ...
  const handleCreate = () => {
    ...
    setFormData({
      stage: 'Midline',
      passFailStatus: 'Pass',
      buyerQCInvolvement: false,
      defectsFound: [],
    });
    setShowForm(true);
  };

  const handleEdit = (inspection: MidlineEndlineInspection) => {
  ...
  const handleDelete = (inspectionId: string) => {
    if (confirm('Are you sure you want to delete this inspection record?')) {
      setInspections(inspections.filter(i => i.inspectionId !== inspectionId));
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingInspection) {
      merchantStore.updateMidlineEndlineInspection?.(editingInspection.inspectionId, formData);
    } else {
      merchantStore.createMidlineEndlineInspection?.(formData as Omit<MidlineEndlineInspection, 'inspectionId' | 'createdAt'>);
    }
  ...
  const getStageColor = (stage: string) => {
    switch (stage) {
      case 'Midline': return 'bg-blue-100 text-blue-700';
      case 'Endline': return 'bg-purple-100 text-purple-700';
      default: return 'bg-slate-100 text-slate-700';
    }
  };
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold">Midline/Endline QC</h2>
            <p className="text-sm text-slate-500">Manage midline and endline quality control inspections</p>
          </div>
        ...
        <CardContent>
          {inspections.length === 0 ? (
            <div className="py-12 text-center text-slate-500">
              No inspection records found. Create your first inspection to get started.
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50/80">
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium uppercase text-slate-500">Buyer QC</th>
                ...
                <tbody>
                  {inspections.map((inspection) => (
                    <tr key={inspection.inspectionId} className="border-b border-slate-100 hover:bg-slate-50">
                      <td className="px-4 py-3 text-sm font-medium">{inspection.inspectionDate}</td>
                      <td className="px-4 py-3 text-sm">{inspection.styleOrderReference}</td>
                      <td className="px-4 py-3">
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${getStageColor(inspection.stage)}`}>
                          {inspection.stage}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm">{inspection.lineNumber}</td>
                      <td className="px-4 py-3 text-sm">{inspection.inspectorName}</td>
                      <td className="px-4 py-3 text-sm">{inspection.quantityInspected}</td>
                      <td className="px-4 py-3 text-sm">{inspection.buyerQCInvolvement ? 'Yes' : 'No'}</td>
                      <td className="px-4 py-3">
                        <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${getStatusColor(inspection.passFailStatus)}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          ...
                          <Button variant="secondary" size="sm" onClick={() => handleDelete(inspection.inspectionId)}>
          ...
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
            <div className="border-b border-slate-200 p-6">
              ...
              <p className="text-sm text-slate-500">
                {editingInspection ? 'Update inspection details' : 'Create a new midline/endline inspection'}
              </p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-6 p-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Buyer QC Involvement</label>
                  <select
                    className="w-full rounded-lg border border-slate-300 px-3 py-2"
                    value={formData.buyerQCInvolvement ? 'true' : 'false'}
                    onChange={(e) => setFormData({ ...formData, buyerQCInvolvement: e.target.value === 'true' })}
                  >
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium text-slate-700">Report URL</label>
                  <Input
